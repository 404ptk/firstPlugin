        ...
        if (sender instanceof Player player){
            if (cmd.getName().equalsIgnoreCase("directmessage")){
                if (args.length==0){
                    ...
                    if (target!=null){
                        player.sendMessage(ChatColor.DARK_AQUA+"Podaj tresc wiadomosci - "+ChatColor.DARK_AQUA+"/pv [nick] [wiadomosc]");
                    }
                    else {
                    ...
                    if (target != null) {
                        ...
                        target.sendMessage(ChatColor.DARK_GRAY+"["+ChatColor.DARK_AQUA+player.getName()+ChatColor.GRAY+" >> "+ChatColor.AQUA+"ja"+
                                ChatColor.DARK_GRAY+"]: "+ChatColor.GRAY+message);
                        player.sendMessage(ChatColor.DARK_GRAY+"["+ChatColor.DARK_AQUA+"ja"+ChatColor.GRAY+" >> "+ChatColor.AQUA+target.getName()+
                                ChatColor.DARK_GRAY+"]: "+ChatColor.GRAY+message);
                    }
                    else {
                        player.sendMessage(ChatColor.DARK_AQUA+args[0]+ChatColor.RED+" jest offline");
